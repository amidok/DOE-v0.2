...
void UserInterface::edit_fullName()
{
	pair<multimap<string, list<Abiturient>::iterator>::iterator, multimap<string, list<Abiturient>::iterator>::iterator> it;
	it = this->findAbiturient();
	if (it.first == it.second)
		return;
	cout << "Выберите, данные какого абитуриента хотите отредактировать:\n";
	list<Abiturient>::iterator ab = this->chooseAbiturient(it.first, it.second);
	string surname, name, patronymic;
	cout << "Введите фамилию:\n";
	cin >> surname;
	...
	cout << "Введите отчество:\n";
	cin >> patronymic;
	container.edit_fullName(ab, surname, name, patronymic);
	cout << "Отредактировано!\n";
}
void UserInterface::edit_points()
{
	pair<multimap<string, list<Abiturient>::iterator>::iterator, multimap<string, list<Abiturient>::iterator>::iterator> it;
	it = this->findAbiturient();
	if (it.first == it.second)
	...
	do
	{
		...
		if (exam <= 0)
			...
			cout << "Неверный ввод! Общий балл меньше, чем сумма балла аттестата и балла за экзамен!\nПовторите ввод или введите \"12345\" для отмены редактирования!\n";
	} while (exam <= 0 || ((exam + certificate > sum) && exam != 12345));
	if (exam == 12345)
		return;
	container.edit_points(ab, sum, certificate, exam);
	cout << "Отредактировано!\n";
}

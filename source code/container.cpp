...
void Container::edit_fullName(list<Abiturient>::iterator it, string surname, string name, string patronymic)
{
	it->setSurname(surname);
	it->setName(name);
	it->setPatronymic(patronymic);
}
void Container::edit_points(list<Abiturient>::iterator it, double sum, double certificate, double exam)
{
	it->setSum(sum);
	it->setExam(certificate);
	it->setCertificate(exam);
}
